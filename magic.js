...
class Game {
    ...
    progressTurn() {
        ...
        if (this.phase == 7) {
            //Deal damage
            this.players[this.currentPlayer].selectedCards.forEach(attacker => {
        ...
        if (this.phase == 8) {
            //Stop attacking and blocking
            this.players.forEach(player => player.selectedCards = []);
        }
        if (this.phase > 10) { //If it passed the end turn phase
            //Pass the turn
            this.phase = 0;
            this.turn++;
...
class Player {
    constructor(name, deck, game, playerIndex) {
        ...
        this.selectedCards = []; // For declaring attackers and blockers, and whatever stuff comes next
    ...
    updateTurn() {
        ...
        if (this.game.currentPlayer == this.playerIndex) {
            ...
            switch(this.game.phase) {
                case 5: //Declare attack
                    this.action  = ActionType.Attack;
                    break;
                case 6: //Declare block
        ...
        //this.action = this.game.currentPlayer == this.playerIndex ? 'play' : 'wait';
    ...
    canPlaySorcery() {
        return this.canPlayInstant() && this.game.currentPlayer == this.playerIndex && (this.game.phase == 3 || this.game.phase == 9);
    }
    canPlayInstant() {
                ...
                case 'permanents':
                    switch(this.player.action) {
                        ...
                        case ActionType.Attack:
                            if (this.player.selectedCards.includes(this)) {
                                //Stop attacking
                                this.player.selectedCards.splice(this.player.selectedCards.indexOf(this), 1); //remove this from the attackers
                                this.element.classList.remove('attacker');
                                console.log('not attacking');
                            }
                            else {
                                //Declare as an attacker
                                this.player.selectedCards.push(this); //Join the attackers
                                this.element.classList.add('attacker');
                                console.log('Attacking!');
                            }
                            break;
                        case ActionType.Block:
                            let index = this.player.selectedCards.map(v => v.blocker).indexOf(this)
                            if (index >= 0) {
                                //Stop blocking
                                removeLine()
                                this.player.selectedCards.splice(index, 1); //remove this from the blockers
                        ...
                        case ActionType.BlockTarget: 
                            ...
                            console.log('Blocking player '+blockingPlayer.name);
                            ...
                            if (blockingPlayer.temp != undefined) {
                                ...
                                // Add the blocker to their list with this card as its target
                                blockingPlayer.selectedCards.push({blocker: blockingPlayer.temp, target: this});

                                //Draw the line to connect these two
                                adjustLine(this.element, blockingPlayer.temp.element);
                                //Change UI to show the blocker is blocking
                                // blockingPlayer.temp.element.classList.add('blocker');
